    ...
    <div className="navbar flex items-center justify-center">
      <div className="container main-n-wrapper flex items-center justify-between">
        ...
        <div className="right-nav flex items-center justify-end">
          <Link to="/engagements">
            <p className="mr-6 text-lg font-medium text-amber-500 cursor-pointer ">
              Find a skill
            </p>
          </Link>
          <Link to="/jobs">
          ...
          {user !== null ? (
            <>
              ...
              <Link to="/create">
                <button
                  onClick={() => setOpenRegModel(true)}
                  className="bg-black px-8 py-2 border-black border-2 rounded-full text-lg font-medium text-amber-500"
                >
            ...
            <>
              <Link to="/login">
                <button className="mr-6 border-amber-500 border-[1px] rounded-full py-2 px-8 text-lg font-medium">
                  Login
                </button>
              </Link>
              <Link to="/register">
                <button className="bg-black px-8 py-2 border-black border-2 rounded-full text-lg font-medium text-amber-500">
                  Join
                </button>
              </Link>
            </>
        ...
        <p
          ...
          className="bars flex text-3xl text-zinc-600 cursor-pointer"
        >
      ...
      <div className={open ? "mobile-view show" : "mobile-view"}>
        <div className="mobile-i-wrapper px-3 py-3">
          <p
            onClick={() => openMobileMenu()}
            className="mb-8 cursor-pointer font-bold text-xl text-indigo-500"
          >
          ...
          {user ? (
            ...
            <>
              <Link to="/register">
                <button
                  onClick={() => setOpen(false)}
                  className="bg-indigo-700 w-full py-3 -mt-5 px-3 rounded-full mt-6 text-white"
                >
                  Join Skillshub
                </button>
              </Link>
              <Link to="/login">
                <button
                  onClick={() => setOpen(false)}
                  className="border-amber-500 w-full py-3 px-3 rounded-full mt-6 mobile-login-btn"
                >
                  Login here
                </button>
              </Link>
          ...
          <div className="mt-5">
            <Link to="/">
              <p
                onClick={() => setOpen(false)}
                className="mb-2 text-lg font-medium text-zinc-500 cursor-pointer"
              >
                Home
              </p>
            </Link>
