...
function SingleJob({job}) {
    const { isLoading, error, data, refetch } = useQuery({
        ...
        queryFn: () =>
        axios(`https://talented-pink-buckle.cyclic.app/api/user/${job?.userId}`).then((res) => {
            return res.data;
      })
  });
  return (
    <div className='singlejob border-gray-200 single-j-border p-2'>
        <div className='flex flex-col job-wrapper'>
            ...
            <div className='mt-4 flex items-center justify-between location-button'>
               ...
               <Link to={`/job/${job?._id}`}> <button className='job-post-btn py-2 px-10 text-white rounded-full'>More</button></Link>
            </div>
        </div>
    </div>
  )
